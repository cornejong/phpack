...
class File
{
    ...
    public static function recursiveList(string $directory, string $pattern = '/.*/'): array
    {
        ...
        $recursive_directory = new RecursiveDirectoryIterator($path, FileSystemIterator::SKIP_DOTS);
     ...
     * @return bool                     Returns true is all went good, false if not
     */
    public static function rename(string $file, string $new_name, bool $includeExtension = false): bool
    {
        /* Get the real path to the directory */
        $path = self::getPath($file);
        /* Get the file extension */
        ...
        return rename($path, $base_path . $new_name . ($extension && $includeExtension ? '.' . $extension : ''));
    }
