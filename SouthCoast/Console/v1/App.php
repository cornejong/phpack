...
namespace SouthCoast\Console\v1;
...
use SouthCoast\Console\Console\v1;
...
class App // implements SplSubject
{
    ...
    {
        if ($this->eventExits($event)) {
            throw new \Exception('Event "' . $event . '" already exists', 1);
        }

        $this->observers[$event] = new SplObjectStorage;
    }

    public function eventExits(string $event)
    {
        return isset($this->observers[$event]);
    }

    public function notify(string $event)
    {
        if (!$this->eventExits($event)) {
            throw new \Exception('Event "' . $event . '" does not exist', 1);
        }
